// Consecutive Strings - https://www.codewars.com/kata/consecutive-strings/javascript
...
function longestConsec(strarr, k) {
  ...
  if(strarr.length === 0 || strarr.length < k || k <= 0){
    ...
    for(var i = 0; i < strarr.length; i++){
      if(strarr.slice(i, i+k).join('').length > longestStr.length){
        longestStr = strarr.slice(i, i+k).join('');
      }
    }
